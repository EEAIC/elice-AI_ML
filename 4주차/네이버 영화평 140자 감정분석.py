...
def calculate_doc_prob(training_sentence, testing_sentence, alpha):
    ...
    예) 3 * 5 = 15
        ...
        log(5) - log(2) = log(2.5)
    '''
    num_of_tokens = sum(training_model.values())
    for word, token in testing_model.items():
        if word in training_model:
            logprob += token * math.log(training_model.get(word) / num_of_tokens)
        else:
            logprob += token * math.log(alpha / num_of_tokens)
    return logprob
